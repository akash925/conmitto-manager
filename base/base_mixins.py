...
class BaseBulkDeleteMixin(object):
    """
    Mixin that adds an @action at /bulk_delete to delete multiple items.
    request.data is an array of ID's chosen by the user for deletion
    """
    @action(detail=False, methods=['get', 'patch'])
    def bulk_delete(self, request):
        queryset = self.get_queryset()
        if type(self).__name__ == "PersonnelViewSet":
            data = [id for id in request.data if id != request.user.id]
        else:
            data = request.data
        serializer = BulkDeleteSerializer(data={'ids': data})
        serializer.is_valid(raise_exception=True)
        items_to_delete = queryset.filter(id__in=serializer.data['ids'])
...
class BaseLoggingMixin(object):
    ...
    def finalize_response(self, request, response, *args, **kwargs):
        ...
        # print('response context', response.getvalue())
